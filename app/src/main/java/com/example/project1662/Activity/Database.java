...
public class Database extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        String qry2 = "create table admins(username text primary key, password text)";
        sqLiteDatabase.execSQL(qry2);

        // Hardcoded admin creation
        ContentValues adminCV = new ContentValues();
        adminCV.put("username", "admin");
        adminCV.put("password", "pass123$$");
        sqLiteDatabase.insert("admins", null, adminCV);
    }
    ...
    public int adminLogin(String username, String password) {
        ...
        str[1] = password;
        SQLiteDatabase db = getReadableDatabase();
        @SuppressLint("Recycle") Cursor c = db.rawQuery("select * from admins where username=? and password=?", str);
        if (c.moveToFirst()) {
            result = 1;
        }
        return result;
